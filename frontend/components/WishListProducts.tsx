                ...
                {favoriteProduct?.slice(0, visibleProducts)?.map((product: Product) => (
                  <tr key={product?._id} className="border-b">
                    <td className="px-2 py-4 flex items-center gap-2">
                      ...
                      />
                      {product?.images?.[0] && (
                        <Link href={`/product/${product?.slug}`} className="border rounded-md group hidden md:inline-flex">
                          <Image
                    ...
                    <td className="p-2 capitalize hidden md:table-cell">
                      <p className="uppercase line-clamp-1 text-xs font-medium">{product?.category?.name || "—"}</p>
                    </td>
                    <td className="p-2 capitalize hidden md:table-cell">{product?.brand?.name || "—"}</td>
                    <td
